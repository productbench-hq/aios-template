...
A free, MIT-licensed starter kit that turns Claude Code into your personal **AI Operating System (AIOS)**. Built by [Productbench](https://www.productbench.co/) for product managers, senior PMs, and heads of product — handed out at our workshops.
...
1. **Clone this repo** to your own machine (or a private fork — your answers and content stay local, nothing gets sent back to Productbench).
2. **Open it in Claude Code** and run `/onboard`. Answer the 7 questions honestly — voice samples get pasted, not described. About 15 minutes.
3. **Use it for a week.** Bring real questions, make real decisions — Claude logs them to `knowledge/log/` as you go. Hand over real material (interview transcripts, docs) and watch it turn into cited synthesis in `knowledge/pages/`.
4. **Grow it.** A manual task repeats 3+ times → build a skill: follow `references/what-a-good-skill-looks-like.md`, then run `/evaluating-skills` on it. The knowledge base starts to need lifecycle management → add an expansion (`EXPANSIONS.md` tells you which, when).
5. **Run `/weekly-review` once a week.** Fix the system, not the output.
...
├── LICENSE
├── context/                 ← About you and your world: about-me, about-business, priorities,
│                                voice, personalities, connections.md, intake.md (/onboard source)
